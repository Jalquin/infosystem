...
@section('title', 'Upravit osobu ' . $person->name)
...
@section('content')
    ...
    <h2>Upravit osobu <b>{{ $person->name }}</b></h2>
    ...
    <form action="{{ route('people.update',$person->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input id="name" type="text" name="name" value="{{ $person->name }}" class="form-control"
                   placeholder="Jméno" required>
        </div>
        <div class="form-group">
            <label for="email">Email:</label>
            <input id="email" type="email" name="email" value="{{ $person->email }}" class="form-control"
                   placeholder="Email">
        </div>
        <div class="form-group">
            <label for="phone">Telefon:</label>
            <input id="phone" type="tel" name="phone" value="{{ $person->phone }}" class="form-control"
                   placeholder="Telefon">
        </div>
        <div class="form-group">
            <label for="note">Poznámka:</label>
            <textarea id="note" rows="3" name="note" class="form-control"
                      placeholder="Poznámka">{{ $person->note }}</textarea>
        </div>

        <div class="form-group">
            <label for="role-select">Role:</label>
            <select name="role_id" id="role-select">
                <option value="" @if($person->role_id == null) selected @endif disabled hidden>Zvolte roli

                @foreach($roles as $role)
                    <option value="{{$role->id}}"
                            @if($person->role_id == $role->id ) selected @endif>{{$role->name}}</option>
                @endforeach
            </select>
        </div>
            ...
            <select name="addresses[]" id="address-select-multiple" placeholder="Zvolte adresy" multiple>
                @foreach($addresses as $address)
                    <option value="{{$address->id}}"
                            @if(in_array($address->id, $person->addresses->pluck('id')->toArray())) selected @endif>{{$address->street.' '.$address->number}}@if($address->addressType){{' - '.$address->addressType->name}}@endif</option>
                @endforeach
            </select>
        </div>
